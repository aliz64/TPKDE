...
def run_and_time(alg_f, X_sample, iters=float('inf')):
  ...
  print(f"Running and timing algorithm: {alg_f.__name__}")
  if iters != float('inf'):
    print(f"Running for fixed number {iters} of iterations.")
  ...
  print("{}:  d={}, n={} -> |MM(Z)|={}  t={} ms\n".format(
      alg_f.__name__, D, N, len(answer), round(taken_time*1000, 3)))
...
def alg_time_comparison(alg_list, dnlist={3:[10,15,20]}, no_comp=False):
  ...
  for d in dnlist:
    ...
    plt.xlabel("n")
    plt.ylabel(
    f"speedup over {alg_list[0].__name__}" if not no_comp else "time (ms)")
